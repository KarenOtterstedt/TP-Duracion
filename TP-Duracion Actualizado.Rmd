...
```{r}
# - Introducción
# - Inicio análisis descriptivo (qué estamos estudiando "tiempo hasta la complicación", objetivo "estudiar si las cosas afectan el tiempo hasta la complicacion", qué variables "sexo, edad, fumadoreds, actividad fisica, comorbilidades", censuras pueden afectar en la estimación del tiempo)
#     - censuras (no hay tantas) 
#     - Descripción del tiempo según las otras variables
```
...
mutate(sexo = str_to_title(as.character(edad_c))) %>% 
  summarise(media = mean(tiempo),
    ...
    across(where(is.numeric), # Para que sólo use 2 decimales
           ~round(.x, 2))) %>%
  kable(
    caption = "Medidas descriptivas de los tiempos de los pacientes de acuerdo a su edad",
    col.names = c("Edad", "Media", "Mediana", "Desvío estándar"),
    escape = F) %>%
...
mutate(sexo = str_to_title(as.character(sexo))) %>% 
  summarise(mediana = median(tiempo),
            sd = sd(tiempo)) %>% 
    mutate(
    across(where(is.numeric), # Para que sólo use 2 decimales
           ~round(.x, 2))) %>%
  kable(
    ...
    col.names = c("Sexo", "Mediana", "Desvio estandar"),
    escape = F) %>%
  kable_styling(latex_options = "hold_position",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
  column_spec(2, width = "3cm")
...
datos %>% 
  ...
  kable(
    ...
    col.names = c("Actividad", "Mediana", "Desvio estandar"),
    escape = F,
      booktabs = TRUE,
      position = "H",
...
kable(
  ...
  col.names = c("Comorbilidades", "Porcentaje", "Mediana", "Desvío estándar"),
  escape = FALSE
) %>% 
...
```{r evaluacion, fig.cap = 'Residuos de Schoenfeld para edad vs tiempo', echo = F}
