...
graf_actividad_box <- datos %>% 
  group_by(actividad) %>% 
  ggplot(aes(x = actividad, y = tiempo, fill = actividad)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("#92C5DE", "#F4A582", "red"))+
  labs (title ='Niveles de actividad',
        x = "Nivel de actividad",
        y = "Tiempo (en meses)") +
  scale_x_discrete(label = c("Alta", "Baja", "Moderada")) +
  theme_bw() +
  theme(legend.position = "none") 
