...
Para conocer el comportamiento de los datos recolectados, se realiza un análisis descriptivo.


```{r Edades, echo = F,out.width='70%', cache = T}
# Histograma:
ggplot(datos, aes(x = edad_c)) +
  geom_histogram(fill = "#92C5DE", color = "black") +
  theme_bw()+
  xlim = seq(35, 100, length.out = 10)+
...
datos <- datos %>% 
  ...
  mutate(edad_c = case_when(
    (40 <= edad & (edad < 60)) ~ "40 - 60",
    (60 <= edad & (edad< 70)) ~ "60 - 70",
    T ~ "> 70")) %>% 
...
datos %>% 
  group_by(com_c) %>% 
  ggplot(aes(x = com_c, y = tiempo, fill = "orange3")) + 
  geom_boxplot() +
...
lr_1 <- survdiff(Surv(tiempo, complicacion) ~ edad_c, rho = 0,
                 data = datos)
...
```{r graficos-supervivencia, echo = F}
