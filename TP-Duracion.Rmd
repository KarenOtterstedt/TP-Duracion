...
datos %>% 
  ...
  mutate(
    across(where(is.numeric), # Para que sólo use 2 decimales
           ~round(.x, 2))) %>%
  kable(
    caption = "\\parbox{.9\\linewidth}{\\centering Medidas descriptivas de los pacientes de acuerdo a su nivel de actividad física}",
...
datos %>% 
  ...
  mutate(
    across(where(is.numeric),
           ~round(.x, 2))) %>%
kable(
  caption = "\\parbox{.9\\linewidth}{\\centering Medidas descriptivas de los pacientes de acuerdo a la cantidad de comorbilidades que presentaron}",
