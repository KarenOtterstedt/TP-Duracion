...
En principio se realiza un análisis descriptivo de las variables, para ver como es el comportamiento de cada una 

```{r Edades, echo = F,out.width='70%', cache = T}

# Histograma:
ggplot(datos, aes(x = edad)) +
  geom_histogram(fill = "#92C5DE", color = "black") +
  theme_bw()+
  labs(title = 'Histograma de las edades de los pacientes al iniciar su seguimiento',
       y = "Frecuencia",
       x = "Edad")
```
...
datos %>% 
  ...
  labs (title ='No Fumadores vs Fumadores',
        x = "Fumador",
        y = "Tiempo (en meses)") +
...
```{r datos-c, include = F}
# Categorizando comorbilidades (armé un dataset nuevo porque sino pasaba algo raro) y también le puse que queden ordenados los niveles de actividad
datos_c <- datos %>% 
  mutate(
    actividad = str_to_title(as.character(actividad)), 
        actividad = factor(actividad,
...
summary(datos_c$comorbilidades)
```
...
```{r boxplot-comorbilidades, echo = F, cache = T}
datos_c %>% 
  group_by(comorbilidades) %>% 
  ggplot(aes(x = comorbilidades, y = tiempo, fill = "orange3")) + 
  geom_boxplot() +
  labs (title ='Gráfico : comorbilidades',
        x = "Comorbilidades",
        y = "Tiempo (en meses)") +
 # scale_x_discrete(label = c("No", "Si")) +
  theme_bw() +
...
```{r sexoactividad,out.width='90%', echo = F, cache = T}
...
datos_c %>% 
  ...
  labs(title = "Actividad de acuerdo al sexo",
       x = "Sexo",
       y = "Tiempo (en meses)",
       fill = "Actividad") +
  theme_bw() 
...
```{r actividadfumadores, echo = F, out.width='90%', cache = T}
...
datos_c %>% 
  ...
  labs(title = 'Tiempo de los pacientes teniendo en cuenta si son fumadores activos o no y el nivel de actividad física',
       x = "",
       y = "Tiempo (en meses)",
       fill = "Actividad") +
...
# Modelo edad: VER DE CATEGORIZAR EDADES
lr_1 <- survdiff(Surv(tiempo, complicacion) ~ edad, rho = 0,
                 data = datos_c)
...
$$
<!-- Para actividad y para comorbilidades -->

```{r test-tendencia, echo = F}
# Actividad:
ggsurvplot(fit = mod_4, data = datos_c,
           #title = "Figura 10: Probabilidad de supervivencia estimada segun grupo etario",
        #   palette = c("coral", "lightblue", "lightgreen"), 
           ...
           test.for.trend = T,
         #  legend.labs = c("30 anos o menos", 
                           # "Entre 31 y 40 anos",
                           # "Mas de 40 anos"),
           legend.title="")

# ggpubr::ggpar(fig_10, font.legend = list(size = 12, color = "black", face = "bold"))


ggsurvplot(fit = mod_5, data = datos_c,
           #title = "Figura 10: Probabilidad de supervivencia estimada segun grupo etario",
        #   palette = c("coral", "lightblue", "lightgreen"), 
           conf.int = F, 
           censor.shape = 20,
...
<!-- VER QUE DECIR DE SEXO PORQUE ESTÁ AL BORDE -->
...
```{r supervivencia-modelos, include = F}
# Modelos para graficar:

# Modelo Nulo
mod_0 <- survfit(Surv(tiempo, complicacion) ~ 1, data = datos_c)
summary(mod_0)

# Modelo con edad:
mod_1 <- survfit(Surv(tiempo, complicacion) ~ edad, data = datos_c)
summary(mod_1)

# Modelo con sexo:
mod_2 <- survfit(Surv(tiempo, complicacion) ~ sexo, data = datos_c)
summary(mod_2)

# Modelo con fumador:
mod_3 <- survfit(Surv(tiempo, complicacion) ~ fumador, data = datos_c)
summary(mod_3)

# Modelo con actividad:
mod_4 <- survfit(Surv(tiempo, complicacion) ~ actividad, data = datos_c)
summary(mod_4)

# Modelo con comorbilidades
mod_5 <- survfit(Surv(tiempo, complicacion) ~ comorbilidades, data = datos_c)
summary(mod_5)
...
```{r graficos-modelos, echo = F}
# GRAFICOS DE SUPERVIVENCIA:

# Modelo nulo:
ggsurvplot(fit = mod_0, data = datos_c,
           censor.shape = 20,
...
# Modelo con sexo:
ggsurvplot(fit = mod_2, data = datos_c,
           censor.shape = 20,
           xlab = "Meses",
           ylab = "Probabilidad de Supervivencia estimada",
...
# Modelo con fumador:
ggsurvplot(fit = mod_3, data = datos_c,
           censor.shape = 20,
           xlab = "Meses",
           ylab = "Probabilidad de Supervivencia estimada",
...
```{r}
# Lo mismo pero con p-values:
pvalues <- as.data.frame(rbind(-2*modc_1$loglik[1], -2*modc_1$loglik[2], -2*modc_2$loglik[2], -2*modc_3$loglik[2], -2*modc_4$loglik[2], -2*modc_5$loglik[2], -2*modc_6$loglik[2], -2*modc_7$loglik[2], -2*modc_8$loglik[2], -2*modc_9$loglik[2]))

pchisq(-2*modc_1$loglik[2], df = 1, lower.tail = F)
pchisq(-2*modc_2$loglik[2], df = 1, lower.tail = F)
pchisq(-2*modc_3$loglik[2], df = 1, lower.tail = F)
pchisq(-2*modc_4$loglik[2], df = 1, lower.tail = F)
pchisq(-2*modc_5$loglik[2], df = 1, lower.tail = F)
...
# ggsurvplot(fit = mod_1, data = datos_c,
#            censor.shape = 20,
#            xlab = "Tiempo (en meses)",
#            ylab = "Probabilidad de Supervivencia estimada",
#            legend = "none",
...
ggsurvplot(fit = mod_5, data = datos_c,
           ...
           xlab = "Meses",
           ylab = "Probabilidad de Supervivencia estimada",
           legend.labs = levels(datos_c$comorbilidades),
