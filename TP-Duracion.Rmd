...
```{r Edades, echo = F, fig.cap = "Histograma de edades", cache = T}
...
```{r Fumadores, echo  = F, fig.cap="Boxplot de fumadores vs no fumadores", cache = T}
...
```{r Actividad, echo = FALSE, fig.cap = "Medidas descriptivas de actividad", cache = TRUE}
...
datos %>% 
  ...
  mutate(
    across(where(is.numeric), # Para que sólo use 2 decimales
           ~round(.x, 2))) %>%
  kable(
     caption = "\\parbox{.8\\linewidth}{Medidas descriptivas de los pacientes según nivel de actividad física}", # Para que baje el título 
     # Cortaba una palabra antes
     # NO ME CONVENCE COMO QUEDA VER
    col.names = c("Actividad", "n", "Media", "Mediana", "Desvío estándar"),
    escape = F) %>% # No me acuerdo qué hace el escape, pero creo que era para la caption
...
ggplot(datos, aes(x = tiempo, y = comorbilidades)) +
  ...
  theme_bw()
```
En base a @ref(fig:Comorbilidad) se espera que el tiempo medio de supervivencia para personas con mayor numero de comorbilidades, sea menor que para las persona con menos comorbilidades
...
```{r Tabla_Comorbilidad, echo = F, cache = T}
datos %>% 
  ...
  mutate(
    across(where(is.numeric),
           ~round(.x, 2))) %>%
  kable(
    caption = "Medidas descriptivas de los pacientes según cantidad de comorbilidades",
...
```
<!-- VER, no me convence -->
Mucha gente presentó 2, 3 o 4 comorbilidades, y, esos grupos de personas tuvieron tiempos registrados similares.
Sólo 3 personas no tuvieron comorbilidades.



```{r Sexo_actividad física, echo = F, cache = T}
...
Puede verse que hay una gran dispersión entre las mujeres que realizan bajo nivel de actividad física en los tiempos de supervivencia, a diferencia de los hombres, que presentan tiempos mucho más concentrados.
...
```{r Actividad_fumadores, echo = F, cache = T}
